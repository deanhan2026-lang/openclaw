...
const QA_SCENARIO_CATALOG = "extensions/qa-lab/src/scenario-catalog.ts";
...
describe("package artifact reuse", () => {
  ...
  it("runs current trusted Telegram QA harnesses against exact candidate runtimes", () => {
    ...
    expect(releaseStepNames.indexOf("Validate candidate release provenance")).toBeLessThan(
      releaseStepNames.indexOf("Install candidate dependencies"),
    );

    expect(scenarioCatalog).toContain("resolveQaRepoPath(import.meta.dirname, relativePath, kind)");
  });
